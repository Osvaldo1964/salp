...
class EnergiesController
{
	...
	public function create()
	{
		...
		if (isset($_POST["bill"])) {
			...
			if (
				preg_match('/^(0|[1-9]\d*)(\.\d+)?$/', $_POST["bill"]) &&
				preg_match('/^(0|[1-9]\d*)(\.\d+)?$/', $_POST["amount"]) &&
                preg_match('/^(0|[1-9]\d*)(\.\d+)?$/', $_POST["fee"])
			) {

				/* Agrupamos la información */
				$data = array(
					"id_lender_energy" => $_POST["lender"],
					"period_energy" => $_POST["period"],
					"bill_energy" => $_POST["bill"],
					"amount_energy" => str_replace(",","",$_POST["amount"]),
					"fee_energy" => str_replace(",","",$_POST["fee"]),
                    "total_energy" => str_replace(",","",$_POST["total"]),
					"date_created_energy" => date("Y-m-d")
				);

				$url = "energies?token=" . $_SESSION["user"]->token_user . "&table=users&suffix=user";
				$method = "POST";
				$fields = $data;
				...
				if ($response->status == 200) {
					...
					matPreloader("off");
					fncSweetAlert("close", "", "");
					fncSweetAlert("success", "Registro grabado correctamente", "/energies");
				</script>';
				}
			} else {
				echo '<script>
	...
	{
		if (isset($_POST["idEnergy"])) {
			echo '<script>
					matPreloader("on");
					fncSweetAlert("loading", "Loading...", "");
				</script>';

			if ($id == $_POST["idEnergy"]) {
				$select = "id_energy";
				$url = "energies?select=" . $select . "&linkTo=id_energy&equalTo=" . $id;
				$method = "GET";
				$fields = array();
				$response = CurlController::request($url, $method, $fields);
				...
				if ($response->status == 200) {
					...
					if (
						...
						/* Agrupamos la información */
						$data = "id_lender_energy=" . $_POST["lender"] .
							"&period_energy=" . $_POST["period"] .
							"&bill_energy=" . $_POST["bill"] .
							"&amount_energy=" . str_replace(",","",$_POST["amount"]) .
                            "&fee_energy=" . str_replace(",","",$_POST["fee"]) .
						...
						/* Solicitud a la API */
						$url = "energies?id=" . $id . "&nameId=id_energy&token=" . $_SESSION["user"]->token_user . "&table=users&suffix=user";
						...
						if ($response->status == 200) {
							echo '<script>
									...
									matPreloader("off");
									fncSweetAlert("close", "", "");
									fncSweetAlert("success", "Registro actualizado correctamente", "/energies");
							</script>';
						} else {
							echo '<script>
									fncFormatInputs();
